...
class MaskDINO(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        if dn == "det":
            ...
            weight_dict.update({k + f"_dn": v for k, v in weight_dict.items() if k!="loss_bbox" and k!="loss_giou"})
        ...
        if cfg.MODEL.MaskDINO.BOX_LOSS and cfg.MODEL.MaskDINO.MASK_LOSS:
            losses = ["labels", "masks","boxes"]
        elif cfg.MODEL.MaskDINO.MASK_LOSS and not cfg.MODEL.MaskDINO.BOX_LOSS:
            losses = ["labels", "masks"]
        elif not cfg.MODEL.MaskDINO.MASK_LOSS and cfg.MODEL.MaskDINO.BOX_LOSS: 
            losses = ["labels", "boxes"]
    ...
    #             # tar_bbox = torch.sigmoid(bbox_pred)

    #             new_outputs.append({'labels': tar_cls.clone(), 'masks': tar_mask.clone()}) #'boxes': tar_bbox.clone()
    #     return new_outputs
    ...
    def forward(self, batched_inputs,branch = 'supervised',return_preds = False):
        ...
        if branch == 'supervised': 
            ...
            # print("orginal image shape",images.tensor.shape)
            if self.training:
                ...
                processed_results = []
                # for mask_cls_result, mask_box_result, input_per_image, image_size in zip(
                #     mask_cls_results, mask_box_results, batched_inputs, images.image_sizes
                # ):
                for mask_cls_result, mask_pred_result, mask_box_result, input_per_image, image_size in zip(
                    ...
                    new_size = mask_pred_result.shape[-2:] #image_size #mask_pred_result.shape[-2:] #image_size  # padded size (divisible to 32)
                    # print("newsize = padded mask size",new_size)
                    ...
                    if self.instance_on:
                        mask_box_result = mask_box_result.to(mask_pred_result)#mask_pred_result
                        height = new_size[0]/image_size[0]*height
                        width = new_size[1]/image_size[1]*width
                        mask_box_result = self.box_postprocess(mask_box_result, height, width)

                        instance_r = retry_if_cuda_oom(self.instance_inference)(mask_cls_result,mask_pred_result,mask_box_result)#mask_pred_result
                        processed_results[-1]["instances"] = instance_r

                return processed_results
        elif branch == 'semi-supervised':
            images = [x["image_aug"].to(self.device) for x in batched_inputs['data']]
            images = [(x - self.pixel_mean) / self.pixel_std for x in images]
            images = ImageList.from_tensors(images, self.size_divisibility)
            ...
            outputs,mask_dict = self.sem_seg_head(features,targets= batched_inputs['pseudo_label'])
            # bipartite matching-based loss
            
            losses = self.criterion(outputs,  batched_inputs['pseudo_label'],mask_dict)
            # print(self.criterion.weight_dict)
            for k in list(losses.keys()):
                if k in self.criterion.weight_dict:
    ...
    def instance_inference(self, mask_cls, mask_pred,mask_box_result):#mask_pred
        # mask_pred is already processed to have the same shape as original input
        image_size = mask_pred.shape[-2:]# [1024,1024] #mask_pred.shape[-2:]
        # print("image size in instance inference",image_size)
        ...
        # print("mask prediction shape",mask_pred.shape)

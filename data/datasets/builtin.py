...
_PREDEFINED_SPLITS ={}
_PREDEFINED_SPLITS['chestxdet'] = {
    # point annotations without masks
    "chestx_instance_train": (
        "train_data/train",
        "annotations/ChestX_Det_train_coco.json",
    ),
    "chestx_instance_val": (
        "test_data/test",
        "annotations/ChestX_Det_test_coco.json",
    ),
}
...
CHX_CATEGORIES = [{ "id": 1, "name": "Atelectasis" },
    ...
    { "id": 10, "name": "Mass" },
    { "id": 11, "name": "Nodule" },
    { "id": 12, "name": "Pleural Thickening" },
    { "id": 13, "name": "Pneumothorax" }]



def _get_ade_instances_meta():
    thing_ids = [k["id"] for k in CHX_CATEGORIES]
    assert len(thing_ids) == 13, len(thing_ids)
    # Mapping from the incontiguous ADE category id to an id in [0, 99]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in CHX_CATEGORIES]
    ret = {
...
def register_all_chestxdet(root):
...
if __name__.endswith(".builtin"):
    # Assume pre-defined datasets live in `./datasets`.
    _root = os.path.expanduser(os.getenv("DETECTRON2_DATASETS","C:/Users/aniru/Desktop/jliang_lab/datasets/chestxdet/dataset/"))# "datasets"))
    register_all_coco(_root)
    register_all_coco_unlabel(_root)
    register_all_chestxdet(_root)
